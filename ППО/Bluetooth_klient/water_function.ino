...
{
  if (flag == 1 && block == 0) // Можем менять режим только если пришла команда с верх уровня (флаг), насос НЕ в работе, и режим НЕ ремонт
  {
    switch (Mode){
    case 0:
    ModeFin = 0;
    client.publish("/sensors/edd0c1b4-9f30-40f0-b31d-6bea2d618666", "Ручной режим");
    button = 0; // При смене режима нужно выключить кнопку включения насоса
    flag = 0;
    break;
    case 1:
    button = 0; // При смене режима нужно выключить кнопку включения насоса
...
void WateringControl() //Управляет поливом (включает, выключает ИМ в соответствии с режимом работы)
{
  ...
  switch(ModeFin)
  {
    ...
    //   Serial.println("Мы везде подаем high но после 10 минут");
      ...
      block = 0;        // Снимаем запрет на смену режима 
      tmr1 = millis(); // Сбрасываем таймер
